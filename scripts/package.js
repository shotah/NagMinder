...
try {
  ...
  try {
    ...
    if (process.platform === 'win32') {
      execSync(`powershell -Command "Compress-Archive -Path './build/*' -DestinationPath '${zipPath}' -Force"`, { stdio: 'pipe' });
      console.log('✅ Package created with PowerShell');
    } else {
      execSync(`cd build && zip -r "../${zipPath}" . -x "*.DS_Store" "*.git*"`, { stdio: 'pipe' });
