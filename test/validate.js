...
if (manifest) {
  ...
  test(
    "Has version",
    manifest.version && manifest.version.match(/^\d+\.\d+\.\d+$/),
  ...
  test(
    "Has background service worker",
    manifest.background && manifest.background.service_worker,
    "Missing background service worker"
  );
  test(
    "Has content scripts",
  ...
  test(
    "Has icons",
    manifest.icons && Object.keys(manifest.icons).length >= 3,
    "Should have at least 3 icon sizes"
  );
...
jsFiles.forEach((file) => {
  if (fileExists(file)) {
    try {
      ...
      // Basic syntax checks
      test(
        `${file} has no obvious syntax errors`,
        !content.includes("import "),
        "ES6 imports not supported in Chrome extensions"
      );
      test(
        `${file} uses chrome APIs properly`,
        content.includes("chrome."),
        `Should use chrome APIs`
      );
      ...
      if (file === "background.js") {
        test(
          `${file} has year tracking`,
          content.includes("year"),
          "Should track yearly stats"
        );
        test(
          `${file} has all-time tracking`,
          content.includes("allTime"),
          "Should track all-time stats"
        );
      }
      if (file === "content.js") {
        test(
          `${file} has expandable UI`,
          content.includes("nm-expand"),
          "Should have expandable stats UI"
        );
      }
    } catch (e) {
...
if (fileExists("styles.css")) {
  ...
  test(
    "CSS has nagminder styles",
    css.includes("#nagminder-bar"),
    "Missing main bar styles"
  );
  test(
    "CSS has high z-index",
    css.includes("z-index") && css.includes("999"),
    "Bar should have high z-index"
  );
}
...
["16", "48", "128"].forEach((size) => {
  ...
  if (fileExists(iconPath)) {
    ...
    test(
      ...
      stats.size > 100,
      `Icon seems too small (${stats.size} bytes)`,
      true
    );
  }
});
...
console.log("\n📊 Summary:");
console.log(
  `✅ Tests passed: ${requiredFiles.length + (manifest ? 9 : 0) + jsFiles.length * 2 + 2 + 3 - errors - warnings}`
);
if (warnings > 0) {
